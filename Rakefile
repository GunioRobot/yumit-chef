...
task :create_dna  do
  dna_file = File.join(File.dirname(__FILE__), "config", ENV['dna'] ? "#{ENV['dna']}.rb" : "dna.rb")
...
task :upload => [:test, :create_dna]  do
  ...
  sh "rsync -rlP --rsync-path='sudo rsync' --delete --exclude '.*' #{File.dirname(__FILE__)}/ #{ENV['server']}:#{REMOTE_CHEF_PATH}"
...
task :cook => [:upload]  do
  ...
  sh "ssh #{ENV['server']} \"cd #{REMOTE_CHEF_PATH}; sudo chef-solo -l debug -c config/solo.rb -j config/dna.json \""
end
